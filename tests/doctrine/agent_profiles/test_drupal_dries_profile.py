"""WP01/T004 — failing verification for the drupal-dries agent profile.

Mission ``drupal-dries-profile-01M28X69``. This is the RED half of
...
class TestDrupalDriesProfileLoads:
    ...
    def test_profile_does_not_claim_the_universal_yaml_language(self, repo: AgentProfileRepository) -> None:
        """``applies_to_languages`` gates the activation catalog; ``yaml`` would
        surface a Drupal specialist in nearly every project. Drupal YAML is
        already covered precisely by ``file-patterns``."""
        # Arrange
        profile = repo.get(_PROFILE_ID)
        assert profile is not None, f"profile {_PROFILE_ID!r} not found"
        # Act
...
class TestMalformedProfileIsObservable:
    def test_malformed_profile_appears_in_skipped_profiles(self, tmp_path: Path) -> None:
        ...
        (built_in / "broken-drupal-dries.agent.yaml").write_text("this: is: not: valid: yaml: {", encoding="utf-8")
