"""ATDD for the ownership-gated profile-context retirement (WP07, census row 7).

Both directions of the asset-preservation guard are pinned for the 2.2.0
migration that removes ``spec-kitty.profile-context.md`` from configured agent
command dirs:

* **preserve** (RED on base ``32cfc272ee``) — a user-authored file that reuses
  the retired command filename but carries no version marker is unprovable, so
  it SURVIVES in place with a diagnostic. On base the migration unlinks it by
  name → this test is RED.
...
def test_user_authored_profile_context_survives_in_place(tmp_path: Path) -> None:
    ...
    result = ProfileContextDeploymentMigration().apply(tmp_path)
