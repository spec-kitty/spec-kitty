"""Contract tests for the canonical no-follow file helpers (issue #699, WP01/#4756).

Canonical home moved to :mod:`kernel.no_follow` (mission
``local-write-safety-01M2ZPZD``): ``kernel.locks`` needs this primitive to
close the lock-path symlink-follow hole and cannot import ``specify_cli``
(the enforced layer direction is ``kernel <- specify_cli``). This module is
the canonical test suite for the kernel primitive; the historical
``tests/specify_cli/core/test_no_follow.py`` stays in place, unmodified, as a
facade-identity contract test proving ``specify_cli.core.no_follow`` still
re-exports the SAME symbols with the same behavior.
"""
...
def test_core_reexport_is_the_same_class_object() -> None:
    """``specify_cli.core.no_follow.NoFollowPathError`` IS this class, not a redefinition.

    ``git_common/gitignore_manager.py:18`` (and 10 other importers) does
    ``except NoFollowPathError`` against the ``specify_cli.core.no_follow``
    import; if the compat shim redefined the class instead of re-exporting
    it, that ``except`` would silently stop matching exceptions raised by
    this kernel module.
    """
    from specify_cli.core.no_follow import NoFollowPathError as CoreNoFollowPathError

    assert CoreNoFollowPathError is NoFollowPathError
