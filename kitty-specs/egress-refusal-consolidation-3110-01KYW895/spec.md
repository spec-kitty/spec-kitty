...
| FR-007 | Project destination authorization is independent of ownership | FR-002 establishes that the acting checkout owns the decision; it does not establish where identifiers may be sent. The earlier fold was incorrect (FU-R / #3178). Before any collaboration request, the canonical hosted admission lookup for the acting checkout's git-origin repository must identify the authenticated team; the returned team slug is the destination. Missing, denied, mismatched or malformed authority refuses before any decision/mission identifier leaves. Environment selectors may refuse a disagreement but never grant authority. | High | independent | See #3178 |
...
| FR-007 | **independent** | **Not discharged by FR-002.** #3178 regression coverage: an owned decision with a foreign or absent repository admission sends no collaboration request; a matching admission uses the server-provided team slug (not its numeric ID). |
| FR-008 | build | SC-015 |
