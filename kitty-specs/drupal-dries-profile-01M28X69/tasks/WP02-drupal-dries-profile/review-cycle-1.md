...
**Work package**: WP02 — Drupal Dries profile artifact
**Commit under review**: `f6c5013ca` (`packs/built-in/agent_profiles/drupal-dries.agent.yaml`, 208 lines, +208/-0, one file)
...
ids. Adding `drupal-dries.agent.yaml` without adding a table row breaks both parametrisations.
...
FAILED ...[readme_path0-Profile ID] - AssertionError:
  ...
  Extra items in the right set: 'drupal-dries'
...
`/Users/nicolas/Projects/spec-kitty/.worktrees/drupal-dries-profile-01M28X69-lane-b`.
...
| WP01 C-P1 assertions | `tests/doctrine/agent_profiles/test_drupal_dries_profile.py` → **7 passed** ✅ |
...
- `tests/doctrine/drg/*` graph-sharding / regen-roundtrip / unknown-kind / loader-fail-closed reds and
  `test_drupal_dries_lineage.py` — **WP07's** (graph regeneration and the curated lineage edge);
