# Data Model: Drupal Dries Agent Profile
...
## E1 — Drupal Dries profile

**Location**: `packs/built-in/agent_profiles/drupal-dries.agent.yaml`
...
|-------|---------------|--------|
| `profile-id` | `drupal-dries` | FR-001 |
| `name` | `Drupal Dries` | FR-001 |
...
| Field | Value |
|-------|-------|
| source | `agent_profile:drupal-dries` |
...
|--------|--------|----------|--------|
| `agent_profile:drupal-dries` | `agent_profile:implementer-ivan` | `specializes_from` | E3 |
| `agent_profile:drupal-dries` | `directive:DIRECTIVE_0{10,24,25,30,34,51}` | `requires` | E1 `directive-references` |
| `agent_profile:drupal-dries` | `tactic:{dependency-hygiene, tdd-red-green-refactor, supply-chain-install-safety, bug-fixing-checklist}` | `requires` | E1 `tactic-references` |
| `styleguide:drupal-conventions` | `agent_profile:drupal-dries` | `suggests` | `java-conventions` precedent |
| `styleguide:drupal-conventions` | `toolguide:drupal-review-checks` | `suggests` | same |
| `styleguide:drupal-security-performance` | `agent_profile:drupal-dries` | `suggests` | same |
...
Drupal-native theming, deferred to Drupal Dries, in the same voice.
...
`charter activate agent-profile drupal-dries` (FR-012). Inactive is the shipped default, and an
