# Contract: Drupal Dries Profile & Lineage
...
```
GIVEN  packs/built-in/agent_profiles/drupal-dries.agent.yaml exists
WHEN   the agent-profile repository loads the built-in pack
THEN   a profile with profile-id "drupal-dries" is present
...
WHEN   the edges from agent_profile:drupal-dries are queried
...
THEN   a requires edge exists from agent_profile:drupal-dries to each
...
AND    Freddy defers Drupal-native theming to Drupal Dries by name
...
```
GIVEN  a project that has not activated drupal-dries
...
WHEN   `charter activate agent-profile drupal-dries` runs
