...
- Adds **Drupal Dries**, a Drupal-specialist implementer agent profile, so Drupal work packages route to an implementer that already knows the framework's conventions instead of a generic one that has to be coached on every task.
...
- Ships **inactive**. Nothing changes for an existing project until an operator runs `spec-kitty charter activate agent-profile drupal-dries`.
...
- `frontend-freddy.agent.yaml` — `avoidance-boundary` gains a framework-generic clause deferring framework-native theming layers to the stack specialist for that framework; `initialization-declaration` drops the sentence "if it runs in a user's browser, it is my concern", which contradicted the boundary. No Drupal-specific wording is added to Freddy.

**Lineage**

- `src/charter/offering/drg/migration/extractor.py` — one `_CURATED_ARTIFACT_EDGES` entry (5 lines) minting `drupal-dries --specializes_from--> implementer-ivan`. Built-in profile lineage lives in that table; the per-profile `specializes-from` field was retired and is rejected by the model.
...
- **Upgrade / migration:** None required. Adoption is opt-in via `spec-kitty charter activate agent-profile drupal-dries`.
- **Operator / reviewer impact:** A project already using Frontend Freddy sees Freddy's declared scope narrow slightly: framework-native theming layers defer to the stack specialist for that framework, and the "if it runs in a user's browser" sentence is gone. For a project with no such specialist this changes nothing in practice. Built-in profile count goes 25 → 26.
...
- **Spec:** `kitty-specs/drupal-dries-profile-01M28X69/spec.md`
- **Plan:** `kitty-specs/drupal-dries-profile-01M28X69/plan.md`
- **Research:** `kitty-specs/drupal-dries-profile-01M28X69/research.md`
- **Review:** `kitty-specs/drupal-dries-profile-01M28X69/verification-report.md` (and per-WP `tasks/WP*/review-cycle-*.md`)
- **PR summary:** `kitty-specs/drupal-dries-profile-01M28X69/pr-summary.md`
...
- **Inactive by default is a deliberate choice.** Unlike the four incumbent stack specialists, this profile is not in `activated_agent_profiles`; adoption is opt-in until maintainers decide otherwise. If it is ever default-activated, the `docs/api/agent_profiles` page, `toc.yml` entry and profile count must move together.
