# PR summary: Drupalling Dries agent profile

Reviewer summary for the pull request adding `drupalling-dries` to the built-in pack.
The PR body is a rendered copy of this file.
...
- `packs/built-in/{agent_profile,styleguide,toolguide}.graph.yaml` and `pack-manifest.yaml` are output of `spec-kitty doctrine regenerate-graph`. The delta is 4 nodes and 14 edges with no removals. Reviewers can regenerate and diff rather than reading these by hand.
...
| `pytest tests/doctrine/{agent_profiles,styleguides,toolguides,drg}/` | 819 passed |
...
| `git diff --stat main -- pyproject.toml uv.lock` | empty |
