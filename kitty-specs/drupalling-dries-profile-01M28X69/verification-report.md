# Verification Report — Drupalling Dries Agent Profile

**Mission**: `drupalling-dries-profile-01M28X69`
**Work package**: WP08 — Closeout verification (T039–T043)
**Verified at**: 2026-09-12
**Verification surface**: `.worktrees/drupalling-dries-profile-01M28X69-lane-g`, branch
`kitty/mission-drupalling-dries-profile-01M28X69-lane-g`
...
(`kitty-specs/drupalling-dries-profile-01M28X69/`), never a lane copy — see Known Issue KI-4.
...
uv run pytest tests/doctrine/agent_profiles/test_drupalling_dries_profile.py \
              tests/doctrine/drg/test_drupalling_dries_lineage.py \
...
| ID | Verdict | Evidence |
|----|---------|----------|
| FR-001 | **MET** | `packs/built-in/agent_profiles/drupalling-dries.agent.yaml` exists, `roles: [implementer]`, `description` distinguishes it as a Drupal specialist. `doctor doctrine --json` built-in pack `discovered_count` 25 → **26**. |
...
| FR-004 | **MET** | Dries `avoidance-boundary` defers "generic browser component authorship (deferred to Frontend Freddy)"; Freddy `avoidance-boundary` defers "Drupal-native theming — Twig templates, `*.libraries.yml` declarations, `Drupal.behaviors`, render arrays and preprocess functions (deferred to Drupalling Dries)". Reciprocal, both by name. |
| FR-005 | **MET** | `extractor.py` `_CURATED_ARTIFACT_EDGES` gains `("agent_profile:drupalling-dries", implementer-ivan, Relation.SPECIALIZES_FROM)`; `agent_profile.graph.yaml` carries the `specializes_from` edge plus six `requires` edges to DIRECTIVE_010/024/025/030/034/051 and four tactic edges. Discipline inherited, not restated. |
...
| FR-012 | **MET** | `.kittify/charter/charter.yaml` `selected_agent_profiles` lists 23 profiles, none of them `drupalling-dries`; `.kittify/agent_profiles_manifest.json` → 0 drupal hits; `.kittify/charter/graph.yml` → 0 drupal hits. Repository left unactivated. |
...
| ID | Verdict | Evidence |
|----|---------|----------|
| NFR-001 | **MET** | `drupalling-dries.agent.yaml` = **209 lines**, inside the declared 140–215 peer band. |
...
| NFR-007 | **MET** | The only pre-existing **profile** modified is `frontend-freddy.agent.yaml`, and its entire diff lies inside `specialization.avoidance-boundary` (one clause appended, existing clauses unreordered). Note: two roster `README.md` index files and `pack-manifest.yaml` also changed — these are generated/bookkeeping surfaces, not profiles, and adding a row for a new profile is required. |
...
| C-002 | **MET** | Lineage is a DRG edge with canonical `<kind>:<id>` endpoint form — `source: agent_profile:drupalling-dries`, `target: agent_profile:implementer-ivan`, `relation: specializes_from`. No per-profile lineage field anywhere in the declaration. |
...
| ID | Verdict | Result |
|----|---------|--------|
| **SC-001** | **MET** | A Drupal work package is assignable in one selection step: `drupalling-dries` is a discovered, valid, implementer-role built-in profile, and the conventions + security/performance + review-checks artifacts supply the Drupal conventions the operator previously hand-fed. |
...
| **C-S7** — styleguide edges follow the peer precedent | **MET (fixed in WP07 — see KI-3)** | `styleguide.graph.yaml` now carries exactly the three required edges and no others: `styleguide:drupal-conventions → agent_profile:drupalling-dries (suggests)`, `styleguide:drupal-conventions → toolguide:drupal-review-checks (suggests)`, `styleguide:drupal-security-performance → agent_profile:drupalling-dries (suggests)`. Mirrors `styleguide:java-conventions`. Zero forbidden edges remain. |
...
header on the descriptor mirroring the one already on `drupalling-dries.agent.yaml` lines 1–2.
...
**Location**: `drupalling-dries.agent.yaml:86` —
...
(`/Users/nicolas/Projects/spec-kitty/kitty-specs/drupalling-dries-profile-01M28X69/`), never
...
|---|---|---|---|---|
| `agent_profile.graph.yaml` | **1** (`agent_profile:drupalling-dries`) | 0 | 11 | 0 |
...
four constituents (`drupalling-dries`, `drupal-conventions`, `drupal-security-performance`,
...
`feat/drupalling-dries-profile` @ `f31dcf115`) to **26** (lane-g) — a delta of exactly one
...
`avoidance-boundary` now defers Drupal-native theming to Drupalling Dries.

### Line counts

`drupal-conventions.styleguide.yaml` is **210** lines (the acceptance matrix's FR-006 row said 208).
`drupalling-dries.agent.yaml` is **208** lines (§NFR-001 of this report said 209). Neither flips a
verdict.
