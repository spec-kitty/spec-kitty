...
- NFR-002
- NFR-003
planning_base_branch: feat/drupalling-dries-profile
merge_target_branch: feat/drupalling-dries-profile
branch_strategy: Planning artifacts for this mission were generated on feat/drupalling-dries-profile. During /spec-kitty.implement this WP may branch from a dependency-specific base, but completed changes must merge back into feat/drupalling-dries-profile unless the human explicitly redirects the landing branch.
...
create_intent:
- tests/doctrine/agent_profiles/test_drupalling_dries_profile.py
...
- **Planning branch**: `feat/drupalling-dries-profile`
- **Final merge target**: `feat/drupalling-dries-profile`
...
1. Create `tests/doctrine/agent_profiles/test_drupalling_dries_profile.py`.
...
3. Assert, in the positive case:
   - a profile with `profile-id` `drupalling-dries` is present
...
**Files**: `tests/doctrine/agent_profiles/test_drupalling_dries_profile.py` (~90 lines)
...
1. Create `tests/doctrine/drg/test_drupalling_dries_lineage.py`. Read `test_builtin_graph_seam.py` in
   that directory first for the graph-loading idiom.
2. Assert:
   - exactly one `specializes_from` edge from `agent_profile:drupalling-dries`, targeting
...
**Files**: `tests/doctrine/drg/test_drupalling_dries_lineage.py` (~80 lines)
...
Check the tests fail **for the right reason**. Run them and read the output: "profile `drupalling-dries`
