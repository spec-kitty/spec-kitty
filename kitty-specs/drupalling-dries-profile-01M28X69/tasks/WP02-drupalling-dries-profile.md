...
title: Drupalling Dries profile artifact
dependencies:
- WP01
requirement_refs:
- FR-001
...
create_intent:
- packs/built-in/agent_profiles/drupalling-dries.agent.yaml
...
# Work Package Prompt: WP02 – Drupalling Dries profile artifact
...
Author `packs/built-in/agent_profiles/drupalling-dries.agent.yaml` — the artifact that makes User
...
```yaml
profile-id: drupalling-dries
name: Drupalling Dries
...
1. `wc -l packs/built-in/agent_profiles/drupalling-dries.agent.yaml` → must be 140–215 (NFR-001).
...
4. `uv run python -c "import yaml,pathlib; yaml.safe_load(pathlib.Path('packs/built-in/agent_profiles/drupalling-dries.agent.yaml').read_text())"`
