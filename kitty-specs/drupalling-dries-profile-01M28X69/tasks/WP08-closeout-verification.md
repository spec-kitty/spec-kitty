...
authoritative_surface: kitty-specs/drupalling-dries-profile-01M28X69/verification-report.md
create_intent:
- kitty-specs/drupalling-dries-profile-01M28X69/verification-report.md
...
1. Check each of the five new artifacts credits amazee.io's `drupal-agents-md` (Vanilla variant):
   ```bash
   for f in packs/built-in/agent_profiles/drupalling-dries.agent.yaml \
...
1. Confirm no charter or config activates the profile by default:
   ```bash
   grep -rn "drupalling-dries" .kittify/ 2>/dev/null || echo "not activated — correct"
   ```
...
3. Confirm the activation path works:
   ```bash
   uv run spec-kitty charter activate agent-profile drupalling-dries --help
   ```
...
**Steps**: Create `kitty-specs/drupalling-dries-profile-01M28X69/verification-report.md` containing:
