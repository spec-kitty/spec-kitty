...
**C-006 rationale (analysis findings D2, N2).** The session harness pins the development branch name `claude/spec-kitty-remediation-wfje22`, and that name substitutes for the branch name that step 2 of the charter's binding mission-merge procedure prescribes (`issue-<n>-<slug>`). This is a recorded deviation from a binding step, not advisory guidance, and no Decision Moment covers it: it is recorded here and in the PR body so the operator sees it before merging. Only the branch name deviates. Every publication invariant holds: nothing is pushed to `main`, publication is one PR targeting `main`, implementers never merge, and the operator merges.
