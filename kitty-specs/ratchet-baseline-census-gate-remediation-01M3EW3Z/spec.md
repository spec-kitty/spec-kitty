...
| FR-006 | Migrate census keys | As a maintainer, I want the 80 `path:line:op` keys in the destructive-op (56), mutation (22) and overwrite (2) gates re-keyed to content identity, with an old→new mapping artefact proving the exempted site set is identical before and after (#5085, DM-01M3EW4T8P). | High | Open |
...
| FR-017 | Split bridge parity suite | As a maintainer, I want the 13 P0 acceptance test functions in `test_bridge_parity.py` moved to a module that does not depend on the two-run oracle fixture, leaving the oracle retirable in isolation (#2631). | Medium | Open |
...
| FR-019 | Follow-up and routing issues | As the operator, I want follow-up issues filed for (a) retiring the bridge-parity oracle after #2633, (b) the silent `model` suppression in the agent-profile schema left unguarded by #3285, and (c) routing the #2972 S8997 subset now that #1842 is closed. | Medium | Open |
...
| C-005 | No production behaviour change | Changes are confined to `tests/`, test helpers, docs, tooling config and mission artefacts; no `src/` behaviour changes. | Technical | High | Open |
