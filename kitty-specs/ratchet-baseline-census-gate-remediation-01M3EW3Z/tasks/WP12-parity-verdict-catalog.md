...
2. Every row carries:
   ...
   - a verdict;
   - for every retire, convert or split row, a **surviving enforcer** (a node ID containing `::`), or a cited `reason:` plus a `mutation:` reference (NFR-006);
   - for every row whose category is a ban or scan, an `empty-target behaviour` cell (SC-005: does the suite fail when its scan target is empty or missing?).
3. `research/parity_catalog_check.py` machine-checks completeness, row shape and **churn fidelity against grounding Part 2**. It exits non-zero on the planning base (no catalog, 0/45) and prints `45/45 OK` at WP end, together with the count of `n/a` churn rows.
...
WP12 is a `planning_artifact` in `lane-planning`: `resolve_workspace_for_wp` resolves it to the **repository-root checkout** (the primary partition), not a lane worktree. Commit there, on the planning branch. Never push and never merge.
...
- **Steps**:
  ...
  4. **Checks.** Each one prints a named failure:
     ...
     - the verdict is one of `keep`, `keep (retire 1 test)`, `keep + consolidate`, `convert`, `retire`, `retire + relocate`, `split`, `consolidate (deferred)` or `fix + retire parts` (T066 uses `keep (retire 1 test)` for 2 rows, so the enum must accept it);
     - **churn fidelity**: parse the grounding Part 2 table (`research/grounding-2631_2972.md`, the `| file | all | src | mass | pcm | pcsrc | ...` table; paths there are relative to `tests/` and may be bold) and assert each catalog row's five churn values **equal** the grounding values for that module. `n/a (shallow)` is accepted **only** for a module absent from Part 2, and such a row's `note` must give the reason. A documented re-measure (new window and HEAD in the header) replaces the grounding values as the reference. Print the number of `n/a` rows. A catalog with no churn numbers must fail;
     - every row whose verdict contains `retire`, `convert` or `split` has a surviving-enforcer cell containing either a node ID (`::`) or both `reason:` and `mutation:`. `TBD`, or any other bare non-empty text, fails;
     - every row whose category cell contains `ban` or `scan` has a non-empty `empty-target behaviour` cell.
...
- **Steps**:
  ...
  2. Write the table with these columns: `suite | category | pins invariant or shape? | all | src | mass | pcm | pcsrc | verdict | surviving enforcer / NFR-006 proof | empty-target behaviour | owning WP / follow-up | note`. Fill `empty-target behaviour` for every ban/scan row from grounding Part 3 and from reading the suite (e.g. "fails: floor ≥ 16 files after WP08", "vacuous on base: scans deleted `src/specify_cli/next`"). Leave it empty for other rows.
  ...
  4. The rows the mission acts on or defers **must** carry these verdicts and enforcers:
     ...
     | `tests/architectural/test_execution_context_parity.py` | keep + consolidate | xfail→WP docstring map and the redundant `missing_seams` arm removed (reason: the kept negative ban already reds on seam rename; mutation: rename both exempt seams → ban reds via `gate.py:218`, `agent_retrospect.py:264`) | WP08 |
     | `tests/review/test_transition_gate_parity.py` | keep (retire 1 test) | reason: `test_wp09_hook_landmine_disposition_is_documented_accurately` tests its own docstring (self-referential); mutation: editing only the docstring reds it with no behaviour change | WP08 |
     | `tests/architectural/test_docs_cli_reference_parity.py` | keep (retire 1 test) | survivor `tests/architectural/test_docs_cli_reference_parity.py::test_visible_paths_match_reference` for `test_retired_check_residual_option_is_absent`, a retired-name tombstone (~72 s) | WP08 |
     ...
     | `tests/charter/test_context_parity.py` | convert | on-disk packs mirror; renamed `tests/charter/test_context_bootstrap_markers.py`; 10 → 0 src-coupling offenders | WP10 |
...
- The checker compares every row's churn values with grounding Part 2 (not just a spot-check), prints the `n/a` count, and each `n/a` row gives a reason. Break it once yourself: change one churn cell and confirm the checker fails naming that row.
- Enforcer cells contain a `::` node ID or `reason:` + `mutation:`; ban/scan rows have an `empty-target behaviour` cell.
...
**Reviewer RED reproduction**: check out the T065 commit (the first WP12 commit on the planning branch) and run `python kitty-specs/ratchet-baseline-census-gate-remediation-01M3EW3Z/research/parity_catalog_check.py --base 3717c7ea`. It must exit non-zero with "catalog missing" or "0/45", not with a Python traceback.

**Requirement coverage** (prose; frontmatter is regenerated by the orchestrator): FR-018, NFR-006; delivers SC-005 (verdict completeness, and the empty-target column for the ban half).
