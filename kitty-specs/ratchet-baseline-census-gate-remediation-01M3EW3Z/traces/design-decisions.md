...
2026-09-26 · python-pedro · WP03 (lane-c): (1) #3206 acceptance 'remove the two entries' now reads 'delete the two ContentDescriptors in test_kernel_no_doctrine_import._PRE_EXISTING_EXEMPTIONS' (the #3206 exit is carried in each descriptor's rationale). (2) #4727 acceptance 'entry removed' now reads 'remove the src/specify_cli/paths/windows_paths.py::_current_platform::sys . platform . startswith ( descriptor line from os-detect-ban-deferred.txt'. Operator to post both tracker comments. (3) D-OP-6: lock-ban (lock-ban-*.txt) and clock CALL: loaders stay shrink-only at ZERO entries; the content form <path>::<qualname>::<token_substring>[::<occurrence>] (clock: CALL:-prefixed) parsed by _content_identity.parse_descriptor_line is the only sanctioned shape; path:line rows raise ValueError naming file and line. Loaders read a new _iter_exemption_entries() -> (filename, line) so rationale = owning file; the clock _iter_exemption_lines() -> list[str] signature is preserved (derived from entries) for the IMPORT: loader and the unowned test_clock_import_ban.py; os-detect/lock-ban _iter_exemption_lines were replaced (monkeypatch plumbing updated). (4) Residual line-shaped loader for WP13 FR-019(d): tests/architectural/test_no_follow_symlinks_apply_ban.py keeps its own inline path:line loader for _exemptions/no_follow_symlinks_apply.txt (pinned empty by equality); out of WP03 scope. (5) Pre-WP01 test_ratchet_positional_anchor_ban.py::test_ct7_real_3206_import_lineno_exemption_stays_green asserts the kernel file still carries ('kernel/schema_utils.py', 88) and imports no ratchet substrate; it goes red on lane-c by design and is replaced by WP01's test_real_kernel_gate_has_no_unexempted_line_pin (subset assertion, [] after WP03).
