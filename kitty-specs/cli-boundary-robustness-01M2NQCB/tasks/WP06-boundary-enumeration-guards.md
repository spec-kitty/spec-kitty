...
title: Enforce discovered JSON contracts and behavioral placeholder guards
dependencies:
- WP03
- WP04
- WP05
...
created_at: '2026-09-16T21:54:38.695712+00:00'
subtasks:
- T028
- T029
- T030
...
authoritative_surface: tests/architectural/test_json_contract_enumeration.py
create_intent:
...
# Work Package Prompt: WP06 — Enforce discovered JSON contracts and behavioral placeholder guards

## ⚡ Do This First: Load Agent Profile

Use `/ad-hoc-profile-load` to load `paula-patterns` before parsing the rest of this prompt.
Profile: `paula-patterns`. Role: `implementer`. Agent/tool: `codex`.
...
Non-vacuous durable guards discover all JSON options, verify the bounded adopted contract, and reject leaked internal placeholders through runtime output.

Priority: P1. Requirement refs: FR-005, FR-006, FR-007, FR-008, FR-011, FR-012, NFR-001, NFR-002, NFR-003, NFR-004, NFR-006, C-001, C-003, C-005, C-006.

- Structurally traverse the real Typer/Click app tree by bool option flag --json, covering names json_output, output_json and json. Include aliases; do not match Python parameter names or hand-select only known-good commands.
- Classify every discovered command as adopted-shape, already-parseable or non-parseable-deferred. Parse allow-list = adopted plus verified-parseable. Adopted errors also require canonical shape and text/JSON exit parity.
- Freeze known non-parseable exclusions with reasons and follow-up evidence. A new unclassified command fails the gate; the gate must not silently auto-defer a new failure. Do not force unrelated existing commands into this mission.
- Drive commands outside a project with valid required arguments or equivalent safe error fixtures; usage errors due to missing required arguments are not a meaningful contract test. Read-only/error fixtures must prevent external operations.
- Empty-success fixtures are explicit and extensible, including zero-WP status, glossary empty list and context lists. JSON payload and exit are asserted separately from mixed output.
- Placeholder guard is behavioral only: invoke each invoke_without_command=True callback with no subcommand and scan both streams for OptionInfo/ArgumentInfo repr. Required floor: root, migrate, context, charter/list callback. The two migrate declarations configure the same callback; discover actual registered surfaces rather than treating those declarations as two callbacks. No AST binding-style gate.
...
- Dependencies: WP03, WP04, WP05. An approved dependency is sufficient; do not wait for done.
- WP01 is the independent P0 and is scheduled first; WP02 remains structurally independent.
- Claim with `spec-kitty agent action implement WP06 --mission cli-boundary-robustness-01M2NQCB --agent codex:<model>:paula-patterns:implementer`.
...
### T028 — Discover and triage the complete JSON command surface

**Purpose:** Discover and triage the complete JSON command surface.

Walk the actual app tree, list flag paths and aliases, and record each classification with its driver/reason. Use the explicit adopted command set from WP02–WP05 as the shape boundary; doctrine.py contributes only mission_type_list, not all unrelated commands in that file. The refined G0/NFR-001 parse allow-list contract governs the earlier universal-parse shorthand. Known residual events.py and implement contamination #2605 belong in deferred inventory, not product edits.
...
### T029 — Commit red-first acceptance for the durable guards

**Purpose:** Commit red-first acceptance for the durable guards.

Replay proposed guards against the unmodified mission baseline in a disposable checkout and capture real contract/placeholder failures, then commit tests before any supporting harness correction. Because this WP follows adoption, also prove the gate fails under a controlled contract regression on its current lane; do not claim a green post-adoption run was red-first.
...
### T030 — Implement the parse and adopted-shape guard

**Purpose:** Implement the parse and adopted-shape guard.

Assert classification completeness, a concrete nonempty discovered-command floor, JSON-only stdout, no raw traceback and canonical adopted errors with command-specific exit parity. Add a self-mutation test demonstrating rejected prose/wrong envelope/new unclassified command; never self-validate via only a static source scan.
...
### T031 — Implement explicit empty cases and callback guard

**Purpose:** Implement explicit empty cases and callback guard.

Use real CLI fixtures for the empty case list. Enumerate all invoke_without_command callbacks, invoke safely without a subcommand, and assert no placeholder representation. Demonstrate failure with injected leaking output, and acceptance of normal real defaults without inspecting declaration style.
...
### T032 — Verify integrated gates and provide traceable closure evidence

**Purpose:** Verify integrated gates and provide traceable closure evidence.

Run both new guards, existing console single-seam gate, bootstrap purity and full architectural tests as required by the charter post-consolidation gate. Give the orchestrator discovery counts, all deferred commands with follow-up titles/IDs, issue-to-test mapping and red/green SHAs. Do not edit product files to make this test-only WP green.
...
```bash
PWHEADLESS=1 .venv/bin/python -m pytest tests/architectural/test_json_contract_enumeration.py tests/architectural/test_cli_placeholder_output.py tests/architectural/test_cli_console_single_seam.py tests/architectural/test_bootstrap_import_purity.py -q
```
...
- Record subtasks with the event-sourced CLI, not markdown checkboxes:
  `spec-kitty agent tasks mark-status T028 T029 T030 T031 T032 --status done --mission cli-boundary-robustness-01M2NQCB`.
- Move WP06 to for_review using the runtime-generated handoff instruction;
