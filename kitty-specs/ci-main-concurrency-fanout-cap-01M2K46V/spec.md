...
> **Documented deviation from ADR literal text (operator-ratified 2026-09-15, post-plan squad).** ADR Axis-2a says "the `report-main` group must move from `cancel-in-progress: false` (queue) toward coalesce-with-survivor." The post-plan adversarial squad found that moving `report-main` to per-SHA coalesce opens a **create-create race** on the single `from:ci` incident issue (two concurrent red tips both create → `len>1` raise → the P0 intake wedges until a human reconciles). Since the fan-out cap is fully delivered by the `types:[completed]` trim + the top-level per-SHA coalesce (which cut `report-main` inflow to ≈1/tip, so its single-group queue now drains trivially), Stage 1 **keeps `report-main` unchanged** and records this as a finding against the ADR's literal mechanic — the ADR's *goal* (queue cannot outgrow drain) is still met. This is within the mission's delegated "exact YAML mechanics" latitude (ADR Confirmation: "medium confidence on the exact YAML/dedup mechanics, the implementing mission's to finalise").
...
- **`report-main` cross-tip serialization preserved**: two concurrent red main tips must not both create a `from:ci` incident issue. `report-main` keeps its single shared group (`cancel-in-progress: false`), so cross-tip main-verdict posts serialize; the fan-out cap makes that queue ≈1/tip so it drains trivially (no queue-outgrows-drain, no create-create race).
- **Terminal survivor has no successor event (residual wedge — detected, self-heal deferred to Stage 3)**: under `types:[completed]` + coalescing, the last upstream completion's run is the survivor and has no later event. If it hits genuine evidence-drift (an upstream re-run bumps `run_attempt` between the two snapshots) or reads before GitHub's API makes the final completion visible, it can post `running`/nothing with no reconciler → a *missing* terminal verdict (never false-green, never blocking-false-red). This residual is **pre-existing in kind** (coalescing removes redundancy margin; it does not introduce the wedge). Stage 1 makes it **detectable** (SC-006) and defers the self-heal — a running-sweep — to Stage 3 (ADR lever 4b).
...
| FR-005 | Main-verdict queue cannot outgrow its drain | As the release-authority contract, I want the `report-main` main-verdict queue to no longer outgrow its drain — achieved by capping `report-main` **inflow** to ≈1 run/tip via the `types:[completed]` trim + top-level per-SHA coalesce, while **preserving** `report-main`'s single-group cross-tip serialization (`cancel-in-progress: false`) so two concurrent tips cannot race to create duplicate `from:ci` incident issues. | High | Open |
...
- **SC-006**: On the merged main tip, **every landed main SHA carries a terminal verdict** — a `[ci] green @<sha>` or `[ci] red @<sha>` (never a stranded `[ci] running @<sha>`) — so the acceptance ledger can *detect* the survivor-no-successor residual wedge rather than pass blind on run-count alone. Measured via `gh` inspection of the PR-comment / incident-issue ledger for each SHA.
