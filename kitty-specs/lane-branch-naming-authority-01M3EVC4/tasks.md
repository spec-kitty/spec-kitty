...
- **Numbering follows execution order across WPs, not WP number.** Wave 1 is {WP01, WP04, WP05, WP06, WP09}. Wave 2 is {WP02, WP10}. Wave 3 is WP03, wave 4 is WP07 and wave 5 is WP08. So WP04's subtasks (T007–T012) come before WP02's (T030–T036).
...
3. **Single gate file** (PD-11 supersedes the plan's Structure Decision / quickstart). WP07 extends `tests/architectural/test_no_worktree_name_guess.py`. There is no sibling `test_lane_naming_authority_gate.py`. The quickstart line naming that file is stale: run `test_no_worktree_name_guess.py` instead.
...
| `src/specify_cli/lanes/recovery.py` | WP04 | (plan: WP03 fallback) | The `_resolve_mission_branch` typed refusal (PD-13) moved into WP04. WP04 also routes its three `mission_id=None` worktree sites through `predict_lane_worktree`, so WP07 never edits the file. |
...
| `src/specify_cli/core/vcs/detection.py` | WP07 | (plan: WP05) | Its `parse_mission_slug_from_branch(f"kitty/mission-{worktree_name}")` line is a **content-pinned carve-out** of the existing gate `tests/architectural/test_no_worktree_name_guess.py` (`_NAME_COMPOSE_BASELINE_RAW_MATCHES = 5`). Changing it in WP05 would turn that gate's staleness guard red in WP05's lane, and the gate is WP07's. WP07 owns both, and changes the site and the allow-list in one commit. |
| `src/specify_cli/lanes/lifecycle_sync.py` `-unknown` error-path placeholder | WP04 (file) | WP07 | The same content-pinned-carve-out reason. WP04 deletes the probe but leaves that one line byte-identical. WP07 removes it as a **documented out-of-map edit** together with the allow-list shrink (serialized by WP07 ← WP04). |

Out-of-map edits allowed: WP07 → `lanes/branch_naming.py` (signature removal + module docstring), `cli/commands/_coordination_doctor.py` (one-line drift matcher) and `lanes/lifecycle_sync.py` (one-line placeholder). WP07 also removes any residual `mission_id=` keyword token found by its AST scan in a non-owned file, and nothing else there. Any other out-of-map edit needs a one-line rationale in the WP's Activity Log.
...
**Prompt**: `/tasks/WP01-reconciliation-claim-created-branch.md` (~313 lines)
...
T004 Add `_unresolvable_approved_lane_branches` refusal and strict approved-lane probes in `build_approved_wp_set` (PD-5) (WP01)
T005 Migrate `tests/merge/test_reconciliation.py` (15 identity-passing calls) to allocator-built lanes (WP01)
T006 Quality gates, blast radius, observe `TestPlanningArtifactReachesTarget` (WP01)
...
**Prompt**: `/tasks/WP04-lifecycle-acceptance-recovery.md` (~257 lines)
**Requirement Refs**: FR-006, FR-007, FR-011, FR-012

### Included Subtasks

T007 [P] Red-first lifecycle-sync test on a divergent shape (created branch attached, no probe/HEAD fallback) (WP04)
T008 Delete `_resolve_lane_branch` / `_git_ref_exists` / HEAD fallback; route `sync_lane_after_coordination_commit` through `predict_lane_worktree`; fix error-path hand-rolled worktree path (WP04)
...
- A function-local import of `predict_lane_worktree` could create an import cycle. If it does, pass `mission_id=None` instead and record the site in the Activity Log for WP07's cutover.
...
- Route five of the seven FR-008 match sites through the parsers: sparse-checkout (plus its hand-rolled compose), status doctor, live-work bindings, commit guard and merge-resolve. The coordination-doctor drift matcher and VCS detection are WP07's (see the ownership table).
...
**Prompt**: `/tasks/WP05-match-sites-parsers-grammar.md` (~312 lines)
...
T014 Add `parse_lane_worktree_dir` and `lane_id_for_worktree_dir`; fix `is_lane_branch` for plain-legacy; parser unit tests (WP05)
...
T018 [P] `policy/commit_guard.py` → `is_lane_branch`; delete the redundant regex in `merge/resolve.py` (WP05)
T019 Site regression tests + quality gates + blast radius (WP05)

### Dependencies

- None. The `_coordination_doctor.py` drift matcher and `core/vcs/detection.py` are **not** in this WP (see the ownership table; WP07 does both).
...
- a materialization failure leaves every file byte-identical;
- a remote-only branch refuses before any write;
- structured JSON is emitted with no traceback.

**Prompt**: `/tasks/WP09-decision-materialize-before-write.md` (~286 lines)
...
**Prompt**: `/tasks/WP02-executor-created-lane-names.md` (~290 lines)
...
T036 Migrate owned executor tests + quality gates + blast radius (WP02)
...
**Independent Test**: `tests/specify_cli/cli/commands/test_coordination_remedy_5113.py` parses the command out of `CoordinationWorktreeUnmaterialized.next_step`, runs it, and asserts the coordination worktree exists.
**Prompt**: `/tasks/WP10-doctor-fix-truthful-remedy.md` (~290 lines)
**Requirement Refs**: FR-014

### Included Subtasks

T037 Red-first remedy round-trip test (`test_coordination_remedy_5113.py`) (WP10)
...
- `TestPlanningArtifactReachesTarget` goes green without editing it.

**Independent Test**: `tests/merge/test_merge_divergent_end_to_end.py` plus the unedited `TestPlanningArtifactReachesTarget`.
**Prompt**: `/tasks/WP03-end-to-end-divergent-merge.md` (~242 lines)
**Requirement Refs**: FR-003, FR-005, FR-011, FR-012

### Included Subtasks

T043 End-to-end `spec-kitty merge` over the 4 divergent shapes (SC-001) (WP03)
...
T047 `TestPlanningArtifactReachesTarget` green unedited + residual-failure triage + quality gates (WP03)
...
## Work Package WP07: Signature cutover + non-vacuous naming gate (Priority: P1)

**Goal**:
- Remove `mission_id` from `lane_branch_name`, `worktree_dir_name` and `worktree_path` atomically with every residual caller (FR-002, PD-1).
- Re-pin the identity-injected lane/worktree goldens (PD-3).
- Migrate the residual test calls.
- Extend `test_no_worktree_name_guess.py` with the signature, compose, match and def-use legs, plus a self-test (FR-009, PD-11), under a shrink-only allow-list in `_baselines.yaml`.
- Route the gate-pinned sites: `core/vcs/detection.py`, the `_coordination_doctor.py` drift matcher, and the `lifecycle_sync.py` placeholder.

**Independent Test**:
- The signature test is red before the cutover.
- The gate is green at 0 compose sites outside the authority.
- Each injected offender form turns the gate red.

**Prompt**: `/tasks/WP07-signature-cutover-and-gate.md` (~355 lines)
**Requirement Refs**: FR-001, FR-002, FR-008, FR-009, NFR-001, NFR-003

### Included Subtasks

T048 Red-first signature test (no `mission_id` on the lane surface) (WP07)
T049 Atomic cutover: remove the parameter in `branch_naming.py` (out-of-map, documented), update the module docstring, then drop every residual `mission_id=None` in src (WP07)
...
T052 Gate-pinned sites: `core/vcs/detection.py` → `parse_lane_worktree_dir`; `_coordination_doctor.py` drift matcher → `lane_id_for_worktree_dir` (out-of-map); `lifecycle_sync.py` `-unknown` placeholder removed (out-of-map); shrink the existing allow-list (WP07)
T053 Extend `test_no_worktree_name_guess.py`: signature, compose, match and def-use legs; allow-list in `_baselines.yaml`; docstring rationale rewritten (WP07)
T054 Gate self-test: injected literal / renamed-variable / `"-".join` / `%`-`.format` / constant-held forms all red (WP07)
T055 Full `tests/architectural/` + quality gates + blast radius (WP07)

### Dependencies

- Depends on WP03, WP04, WP05, WP06, WP10.
...
wave 4: WP07 (←WP03, WP04, WP05, WP06, WP10)
wave 5: WP08 (←WP07)
```

- **Critical path**: WP01 → WP02 → WP03 → WP07 → WP08.
...
- **#5113** (WP09 → WP10) is independent of the naming work. It gates WP07 only through the one-line `_coordination_doctor.py` edit.
...
| T052 | Gate-pinned sites (detection, drift matcher, placeholder) | WP07 | P1 | No |
| T053 | Gate legs + allow-list | WP07 | P1 | No |
| T054 | Gate self-test | WP07 | P1 | No |
| T055 | Architectural suite + gates | WP07 | P1 | No |
| T056 | ADR | WP08 | P3 | No |
