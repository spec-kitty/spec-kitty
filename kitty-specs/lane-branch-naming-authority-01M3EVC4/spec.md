...
| FR-011 | Re-finalize preserves the recorded Mission branch | As an operator, I want re-finalizing tasks to keep the lanes manifest's recorded Mission branch, and first-time finalize to name it exactly as creation does, and every other Mission-branch consumer to prefer the recorded value, so that a backfilled legacy Mission never targets a Mission branch that was never created. | High | Open |
