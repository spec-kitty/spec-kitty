...
- **A pull request that selects no modules (e.g. a test-only or docs-only change) now aggregates green instead of failing on a coverage report it never produced** (#4537). **Before:** with the collector and reconciler fixes in place, a zero-selection PR's `collect` job succeeded with `complete=true` but wrote no coverage XML, so no `ci-aggregate-reconciled-coverage` artifact was uploaded — and the `diff-cover` and Sonar consumers then hard-failed trying to download that absent artifact, reddening the terminal aggregate gate even though every product workflow was green and no module needed scoring. **After:** the reconciler publishes a `has-coverage` signal and the coverage consumers skip deterministically on a *complete empty* selection (the terminal gate already treats a skipped consumer as green); a PR that does select modules still uploads and scores its coverage, and full/nightly mode still fails closed when coverage evidence is genuinely missing.
