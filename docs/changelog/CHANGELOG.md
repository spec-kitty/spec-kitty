...
- **Drupal work now has a specialist agent profile — Drupalling Dries — so a Drupal mission gets an implementer that already knows the framework's conventions instead of a generic one you have to coach.** **Before:** a Drupal work package could only be routed to a generic implementer or a wrong-stack specialist (Java Jenny, Node Norris), so module structure, dependency injection, the Entity API, configuration management and Drupal's test tiers had to be explained by hand on every task, and mistakes surfaced in review rather than being avoided. **After:** `drupalling-dries` ships in the built-in pack as an implementer-role specialist covering Drupal 10.x/11.x backend work (modules, services and DI, Entity API, plugins, hooks, Forms API, routing, configuration management, Batch/Queue/Migration) and Drupal-native theming (Twig, `*.libraries.yml`, `Drupal.behaviors`, render arrays, preprocess). It arrives with two styleguides — `drupal-conventions` and `drupal-security-performance`, carrying all fourteen of the community "never do this" anti-patterns as paired wrong/right examples — and a `drupal-review-checks` toolguide naming the phpcs, phpstan, drupal-check, composer audit and PHPUnit tier commands a reviewer should expect, written so they work whether or not the project runs in a container. Where its territory meets Frontend Freddy's, both profiles now say so explicitly: Dries owns Drupal-native theming, Freddy owns generic browser component work. The profile ships **inactive** — activate it with `spec-kitty charter activate agent-profile drupalling-dries` when you want it. Distilled from amazee.io's community `drupal-agents-md` guide, with one factual error in that source corrected against Drupal's own change record (implicit entity-query access checking was deprecated in 9.2.0 and throws from 10.0.0 — it is not, as the source states, deferred to Drupal 12).
