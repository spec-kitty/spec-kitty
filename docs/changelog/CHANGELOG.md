...
- **A status write for a merged mission whose coordination branch was retired no longer resolves to a dangling deleted branch** (#4401). **Before:** `resolve_placement_only(kind=STATUS_STATE)` for a PUBLISHED (fully-merged) mission whose coordination branch had also been cleaned up returned `CommitTarget(ref="…-coord")` — a branch that no longer exists — instead of refusing, so a later write would target a dead ref. **After:** the write projection fails closed with `ActionContextError` (`COORDINATION_BRANCH_DELETED`), restoring the SC-005 non-regression guarantee `resolution.py` already documented. The regression came in with #4090: restoring the `merged_at` writer correctly flipped these missions to `PUBLISHED`, whose read-path coord probe intentionally degrades to the primary surface — which exposed that the write projection had lost its own fail-closed probe. Read behavior is unchanged (reads of a merged mission still degrade to primary); only new writes to a retired coord ref are refused.
