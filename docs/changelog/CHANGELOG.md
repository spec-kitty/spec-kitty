...
- **`spec-kitty doctor <cmd> --json` printed the human string `Error: Not in a spec-kitty project` on stdout (exit 1) instead of JSON whenever it ran outside a spec-kitty project, so `doctor <cmd> --json | jq` got a parse error indistinguishable from a crash (`#4242`).** The not-in-project guard ran before the `if json_output:` branch and never consulted `json_output`, while the same commands' success path emitted well-formed JSON — a contract inconsistency between exit paths. **Before:** twelve `--json` doctor subcommands leaked prose on stdout on this path (`state-roots`, `workspaces`, `identity`, `topology`, `mission-type`, `shim-registry`, `contracts`, `invocation-pairing`, `ops`, `doctrine`, `cutover`, `review-cycle-reconcile`); `shim-registry`/`contracts` additionally leaked a raw traceback when `locate_project_root` *raised* rather than returned `None`. **After:** all twelve resolve the project root through one shared helper (`_resolve_project_root_or_exit`) that routes both failure modes — raising and returning `None` — through the same `_json_error` envelope the `skills` exemplar already used, so `--json` stdout is a machine-parseable `{"ok": false, "error": {"code": "not_in_project", …}}` object and the human path keeps its Rich prose. Each command keeps its own documented not-in-project exit code (1 for most; 2 for the config-error family `shim-registry`/`contracts`). A parametrized class-closing gate (`tests/specify_cli/cli/commands/test_doctor_json_not_in_project.py`) pins the contract for every affected command. `doctor mission-state` is deliberately unchanged — it treats a `None` root as a valid fixtures-only state, not an error.
