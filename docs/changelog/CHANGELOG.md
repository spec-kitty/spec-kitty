...
- **The automated `spec-kitty next` loop no longer stalls by repeatedly handing out a dependency-blocked work package while ready work sits available** (#4879; closes #4860). **Before:** advancing mode (`spec-kitty next` driving the loop forward) picked the next `implement` work package by filename/lane order alone, so on a mission where a lower-numbered WP was blocked on an unmet dependency it kept re-issuing that blocked WP — even though query mode (`spec-kitty next` with no result) and `agent action implement` already skipped it and selected a ready, higher-numbered one. The two answers disagreed and the canonical loop wedged, forcing an operator to hand-pick a WP to escape. **After:** advancing mode selects through the same dependency-aware claimability authority (`preview_claimable_wp`) that query mode and action auto-selection already use, so all three agree: ready independent work is chosen before blocked lower-numbered work, and a mission with every planned WP genuinely blocked now surfaces an honest `blocked` decision instead of a false "implement WPnn" that can never proceed. Already-active (in-progress) WPs still resume first and the review fallback is unchanged. Red-first coverage pins advancing/query/action agreement both initially and after a dependency is reopened.
