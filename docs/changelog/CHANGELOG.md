...
- **`spec-kitty intake <plan>` no longer silently overwrites an existing `.kittify/mission-brief.md` when its provenance sidecar is absent — a missing `brief-source.yaml` is now treated as unknown provenance and refused without `--force`** (#4910). **Before:** both overwrite gates (the `--auto` scan path and the explicit-path/stdin path) keyed on `brief_path.exists() AND source_path.exists()`, so a brief the operator wrote by hand — or one whose sidecar was deleted or never committed — failed that conjunction and was replaced with the ingested plan on a plain `intake <doc>`: exit 0, `✓ Brief written…`, no `--force`, no backup, the hand-authored text gone. Only the both-files-present shape was ever protected. **After:** both gates key on the brief's existence alone; a present brief refuses with `Brief already exists at .kittify/mission-brief.md. Use --force to overwrite.` (exit 1) and is left byte-identical regardless of the sidecar. The genuine partial-state case — an orphan `brief-source.yaml` with no brief — is unchanged: it is still treated as recoverable partial state and `write_mission_brief()` cleans it before the fresh write. `--force`, `--show`, and the size-cap/identity guards are untouched. Red-first CLI coverage drives the real Typer `intake` command over both gate paths for the missing-sidecar case and pins the orphan-sidecar recovery path.
