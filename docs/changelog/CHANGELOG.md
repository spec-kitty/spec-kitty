...
- **Org template installation preserves the prior pack on failed replacement (#2719).** Rendering now stages beside the destination and promotes the complete tree with a same-filesystem rename. A failed forced promotion restores the original pack, preserving and naming any interrupted destination or recovery backup. Overlapping source/output paths, destination symlinks, option-like Git refs, and malformed template URLs are refused before installation.
