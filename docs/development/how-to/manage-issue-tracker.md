...
| `taken-by-human` | A human contributor (not a fleet VM) owns the linked PR. **The only label that keeps the fleet's PR-routed fix and recovery lanes off human-driven work:** those lanes select on the PR's own labels and never see the linked issue's `status:claimed`, so this — not `status:claimed` — is what holds a fleet implementer VM off your PR (`PROGRAM.md` §13). (Initial admission is separate: the dispatcher already skips issues that carry `status:claimed`.) Pair it with `status:claimed` on the issue when you begin. | The human contributor; `bin/take-next-ticket.sh` sets it automatically for fleet-assisted human claims |
