...
#: #4327: the #3954 interim wire-side bound (``_bound_wire_review_ref`` /
#: ``_looks_like_review_ref_pointer``) is deleted — ``review_ref`` is
#: pointer-only at every producer (``move-task`` never falls back to the
#: operator's ``--note`` prose), so the value reaches the codec verbatim and
#: the codec's own 240-UTF-8-byte attr bound is the single authority again:
#: an over-bound value fails closed loudly in ``_broadcast_moment``'s
#: ``ZeitgeistAttrsError`` handling (a logged WARNING, never a silent drop),
#: never silently truncated or corrupted here.
